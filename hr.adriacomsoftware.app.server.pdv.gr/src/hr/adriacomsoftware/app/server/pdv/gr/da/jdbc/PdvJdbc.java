package hr.adriacomsoftware.app.server.pdv.gr.da.jdbc;
...
public final class PdvJdbc extends OLTPJdbc {
	...
	public PdvJdbc() {
		setTableName("pdv_gr_racun");
	}

	public PdvVo daoLoad(PdvVo value)  {
		J2EESqlBuilder sql = new J2EESqlBuilder();
		sql.append("select * from dbo.fn_tbl_pdv_gr_racun_lista() "
	...
	public String daoZadnjiRedniBroj(PdvVo value)  {
		J2EESqlBuilder sql = new J2EESqlBuilder();
		sql.append(" select isnull(max(redni_broj),0)+1 as max_redni_broj from pdv_gr_racun ");
		try{
			PreparedStatement pstmt = getConnection().getPreparedStatement(sql.toString());
			pstmt.setMaxRows(1);
			PdvVo j2eers = new PdvVo(transformResultSetOneRow(pstmt.executeQuery()));
	...
	public PdvVo daoPronadiVrstuUslugeDetalji(PdvVo value)  {
		J2EESqlBuilder sql = new J2EESqlBuilder();
		sql.append("SELECT * FROM dbo.bi_pdv_konto_protukonto "
				+ "WHERE tip = 'gr' and vrsta_usluge like '"
				+ value.getVrstaUsluge() + "' ");
		try{
			PreparedStatement pstmt = getConnection().getPreparedStatement(sql.toString());
			pstmt.setMaxRows(0);
			PdvVo j2eers = new PdvVo(transformResultSetOneRow(pstmt.executeQuery()));
